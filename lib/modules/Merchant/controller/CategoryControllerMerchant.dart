...
class CategoryControllerMerchant extends GetxController {
  @override
  void onInit() async {
    super.onInit();
    ...
    print("START CategoryControllerMerchant");
    await getCategories();
  }
  ...
  void onClose() {
    super.onClose();
    print("STOP CategoryControllerMerchant");
  }
  ...
  void updateReorderPayload({required int start, required int end}) {
    for (int i = start; i <= end; i++) {
      final product = listCategories[i];
  ...
  getCategories() async {
    ...
    var response = await data.getCategories(
  ...
  createCategories() async {
    if (formstate.value.currentState!.validate()) {
      ...
      var response = await data.createCategories(
  ...
  updateCategories(String id) async {
    if (formstate.value.currentState!.validate()) {
      statusRequestButtonAdd.value = StatusRequest.loading;
      var response = await data.updateCategories(
        name: nameController.text,
        description: descriptionController.text,
        imageCategorie: imageCategorie.value ?? "",
        isActive: isActiveAdd.value,
  ...
  removeCategories(String id) async {
    bool? confirm = await showConfirmationDialog(
      Get.context,
      title:
          "هل تريد حذف (${listCategories.firstWhere((element) => element['_id'] == id)['name']}) ؟",
    ...
    indexLoding.value = listCategories.indexWhere(
      (element) => element['_id'] == id,
    );
    var response = await data.removeCategories(id);
    if (handlingData(response) == StatusRequest.success) {
      listCategories.removeWhere((element) => element['_id'] == id);
